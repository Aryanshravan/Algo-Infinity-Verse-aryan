...
function revealAnswer() {
  ...
  const optionsHtml = card.options.map((opt, i) =>
    `<div class="fp-option" data-option-index="${i}">
      <span class="fp-option-indicator">${String.fromCharCode(65 + i)}</span>${escapeHtml(opt)}
    </div>`
  ).join('');
...
function handleOptionClick(e) {
  ...
  const explanationHtml = card.explanation
    ? `<div class="fp-explanation"><strong>Explanation:</strong> ${escapeHtml(card.explanation)}</div>`
    : '';
